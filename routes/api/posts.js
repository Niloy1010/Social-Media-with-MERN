...
router.post(
  ...
  (req, res) => {
    ...
    const newPost = new Post({
      ...
      user: req.user.id,
      hasImage: true,
      image: `/api/users/profilepicture/${req.file.id}`,
    });
    ...
    Profile.findOne({ user: req.user.id })
      .then((profile) => {
        Post.findById(req.params.id)
          ...
          .catch((err) =>
            res.status(400).json({ status: "could not find post" })
          );
      })
            ...
            .then((user) => {
              if (
                user.notifications.filter(
                  (notification) =>
                    notification.postId.toString() === post.id &&
          ...
          .then((user) => {
            ...
            user.save().then(() => {
              ...
              post
                ...
                .catch((err) =>
                  res.status(400).json({ error: "cannot comment" })
                );
            });
          })
