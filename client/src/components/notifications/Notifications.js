...
class NotificationsComponent extends Component {
    ...
    render() {
        ...
        const recentNotifications = notifications.filter(notification=> {
            ...
            let measureNotificationRecent = (((curDate.getTime() - date.getTime())/60000)/60)/24;
        ...
        if(notifications){
        showNotifications = notifications.reverse().map(notification=> <NotificationItem key={notification._id} notification={notification} />);
        }

        let showRecentNotifications;
        let showOlderNotifications;
        if(notifications) {
            showRecentNotifications = recentNotifications.reverse().map(notification=> <NotificationItem key={notification._id} notification={notification} />);
        }
        if(notifications) {
            showOlderNotifications = olderNotifications.reverse().map(notification=> <NotificationItem key={notification._id} notification={notification} />);
        }
